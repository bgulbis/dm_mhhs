...
write_rds(data_comorbid, "data/tidy/2018-01/data_comorbidities.Rds", "gz")

# diabetes types ---------------------------------------

# Type 1 = E10; Type 2 = E11; Other = E13; (MODY = E13.9); NDM = P70.2
icd_dm <- list(
    DM1 = icd_children(as.icd10("E10")),
...
write_rds(data_dm_diag, "data/tidy/2018-01/data_dm_diagnosis.Rds", "gz")
...
data_a1c <- read_data(dir_raw, "labs-a1c", FALSE) %>%
    ...
    mutate(
        low_a1c = lab.result <= 8 | censor.low,
        high_a1c = !low_a1c
    ) %>%
    select(-low_a1c)

write_rds(data_a1c, "data/tidy/2018-01/data_a1c.Rds", "gz")
